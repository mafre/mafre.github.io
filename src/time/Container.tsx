...
export default function Container(props: {children: React.ReactNode, svgWidth?: number, svgHeight?: number, overlay?: boolean}) {
	...
	const style:React.CSSProperties = overlay ? { ...defaultStyle, position: 'absolute', left: 0, top: 0, pointerEvents: 'none' }: defaultStyle;

	return <svg viewBox={`-${svgWidth/2} -${svgHeight/2} ${svgWidth} ${svgHeight}`} width={svgWidth} height={svgHeight} style={style} imageRendering='optimizeQuality'>
		{children}
	</svg>
}