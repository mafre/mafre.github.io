...
class TimeUtils {
	...
	axisY() {
		return this.innerH / 2;
	}

	/** Build tick values from -range..+range with a given step (hours) */
	ticks(stepHours = 1) {
		const arr: number[] = [];
		for (let h = -this.rangeHours; h <= this.rangeHours + 1e-9; h += stepHours)
			arr.push(Number(h.toFixed(6)));
		return arr;
	}
	...
	degreeToDate(degree: number): Date | null {
		...
		const decl = this.deg2rad(
			23.44 * Math.sin(this.deg2rad((360 / 365) * (this.dayOfYear(now) - 81)))
		);
		const hourAngle = Math.acos(
			(Math.sin(this.deg2rad(degree)) - Math.sin(latRad) * Math.sin(decl)) /
		...
		date1.setHours(
			Math.floor(time1),
			Math.floor((time1 % 1) * 60),
			Math.floor(((time1 * 60) % 1) * 60),
			0
		);
		const date2 = new Date(now);
		date2.setHours(
			Math.floor(time2),
			Math.floor((time2 % 1) * 60),
			Math.floor(((time2 * 60) % 1) * 60),
			0
		);
		// Return the closest time to now
		return Math.abs(date1.getTime() - now.getTime()) < Math.abs(date2.getTime() - now.getTime())
			? date1
			: date2;
	}
