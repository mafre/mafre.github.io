...
export default class Gear {
  ...
  const rot = 0;
    ...
    // bore (hole)
    const bore = boreRadius > 0
      ? <circle cx={cx} cy={cy} r={boreRadius.toFixed(3)} fill="none" stroke={stroke} strokeWidth={strokeWidth} />
      : null;

    // optional pitch circle
    const pitch = showPitch
      ? <circle cx={cx} cy={cy} r={(pitchRadius ?? (rootR + tipR) / 2).toFixed(3)} fill="none" stroke={stroke} strokeDasharray="2,2" strokeWidth={Math.max(0.5, strokeWidth * 0.75)} />
      : null;
  ...
  return <g {...attrs} transform={`rotate(${this.rotation} ${cx} ${cy})`}>
    {/* Position mask to the gear's image box so coordinates match exactly */}
    <mask id={`mask-${this.id}`} maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse" x={cx - radius} y={cy - radius} width={radius * 2} height={radius * 2}>
      ...
      {pitch && <circle cx={cx} cy={cy} r={(pitchRadius ?? (rootR + tipR) / 2).toFixed(3)} fill="white" />}
      {boreRadius > 0 && <circle cx={cx} cy={cy} r={boreRadius.toFixed(3)} fill="black" />}
    </mask>
    <image x={cx - radius} y={cy - radius} width={radius * 2} height={radius * 2} href="gear.png" mask={`url(#mask-${this.id})`} />
  </g>;
  }
}