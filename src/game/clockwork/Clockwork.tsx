...
export class Clockwork {
		...
		while (q.length) {
			...
			for (const e of edges) {
				...
				if (e.type === 'coaxial') {
					...
					const curToothIndex =
						Math.round((contactAngle - (cur.rotation * Math.PI) / 180) / curToothAngle) % curTeeth;
	...
	solveRotations(seedId: string, seedRotationDeg: number = 0): void {
		const seed = this.gears.get(seedId);
		if (!seed) throw new Error('unknown seedId');
		const debugPropagation = false; // set true to emit detailed propagation logs
			...
			for (const gearId of groupGears) {
				...
				for (const e of edges) {
					...
					if (e.type === 'mesh') {
						...
						if (debugPropagation && typeof window !== 'undefined') {
							console.debug(
								`[solveRotations] mesh ${srcGear.id}->${nxt.id} teeth ${srcGear.teeth}/${nxt.teeth} ratio=${ratio.toFixed(4)} deltaMaster=${deltaMaster.toFixed(4)} deltaN=${deltaN.toFixed(4)} newRot=${newRotation.toFixed(4)}`
							);
						}
						q.push(nxtAxle);
						...
						if (debugPropagation && typeof window !== 'undefined') {
							console.debug(
								`[solveRotations] coaxial ${srcGear.id}<->${nxt.id} deltaMaster=${deltaMaster.toFixed(4)} newRot=${(initN + deltaN).toFixed(4)}`
							);
						}
						q.push(nxtAxle);
					}
	...
	renderSVG(
		...
		const gears = Array.from(this.gears.values()).sort(
	...
	propagateDelta(seedId: string, deltaDeg: number): void {
		if (Math.abs(deltaDeg) < 1e-9) return; // nothing to do
		const seed = this.gears.get(seedId);
		if (!seed) throw new Error('unknown seedId');
		...
		deltaByAxle.set(seedAxle, deltaDeg); // external advancement

		const visited = new Set<string>();
		const q: string[] = [seedAxle];

		while (q.length) {
			const axle = q.shift()!;
			if (visited.has(axle)) continue;
			visited.add(axle);
			const sourceDelta = deltaByAxle.get(axle)!; // accumulated delta for this axle

			// For each gear in this axle, propagate to neighbors in other axles
			const groupGears = Array.from(axleGroups.get(axle) || []);
			for (const gearId of groupGears) {
				const src = this.gears.get(gearId)!;
				const edges = adj.get(gearId) ?? [];
				for (const e of edges) {
					const dst = this.gears.get(e.b)!;
					const dstAxle = dst.axleId ?? dst.id;
					if (dstAxle === axle) continue; // same group
					if (deltaByAxle.has(dstAxle)) continue; // already assigned (first path wins)

					let propagated: number;
					if (e.type === 'coaxial') {
						propagated = sourceDelta; // same direction & magnitude
					} else {
						// mesh
						...
						const ratio = useTeeth
							...
							: src.ratio / dst.ratio;
						propagated = -sourceDelta * ratio; // invert direction
					}
		...
		for (const [axle, delta] of deltaByAxle.entries()) {
			if (axle === seedAxle) continue; // seed already rotated
			for (const gid of axleGroups.get(axle) || []) {
				const g = this.gears.get(gid)!;
