...
export default class GearFactory {
  ...
  /**
   * Create a meshing pair with explicit tooth counts rather than ratio math.
   * Useful for documented gear trains where counts are predetermined.
   */
  static createExplicitMesh(
    cw: Clockwork,
    masterId: string,
    newId: string,
    masterTeeth: number,
    ...
    cw.generateGear({
      ...
      teeth: newTeeth,
      axleId: opts.axleB,
      layer: opts.layerB ?? cw.getGear(masterId)!.layer,
      toothRootRatio: opts.toothRootRatio,
      toothTopRatio: opts.toothTopRatio,
      toothDepth: opts.toothDepthB,
    });

    cw.connect(masterId, newId, "mesh", opts.backlash ?? 0);

    return { masterId, newId, masterTeeth, newTeeth };
  }
  ...
  static createExplicitCoaxial(
    ...
    cw.connect(masterId, newId, "coaxial", 0);
