...
export default function SunCurve() {
	const { data } = useGameData();
	const { updatedAt } = data;
	const now = useMemo(() => new Date(updatedAt), [updatedAt]);
	const sampleMinutes = 2;
	const scale = 0.6;
	const timeUtils = useMemo(
		() => new TimeUtils({ height: DEFAULT_SVG_HEIGHT, width: DEFAULT_SVG_WIDTH }),
		[]
	);
	const xOf = (rh: number) => ((rh + 12) / 24) * DEFAULT_SVG_WIDTH * 0.8 + DEFAULT_SVG_WIDTH * 0.1;
	const yOfAlt = (alt: number) =>
		DEFAULT_SVG_HEIGHT * scale * (1 - (alt - altMin) / (altMax - altMin));
	const { sunPts, altMin, altMax } = useMemo(() => {
		...
		const sunPts = relHours.map((rh) => ({
			rh,
			alt: timeUtils.solarAltitudeDeg(timeUtils.addHours(now, rh)),
		}));
	...
	const sunPath = useMemo(
		() =>
			sunPts
				.map(
					(p, i) =>
						`${i ? 'L' : 'M'}${xOf(p.rh)},${yOfAlt(p.alt) + DEFAULT_SVG_HEIGHT * ((1 - scale) / 2)}`
				)
				.join(' '),
		[sunPts, altMin, altMax, DEFAULT_SVG_WIDTH, DEFAULT_SVG_HEIGHT]
	);
	...
	function CurveSun(): React.ReactNode {
		return (
			<g
				transform={`translate(${xOf(0)}, ${yOfAlt(sunAltNow) + DEFAULT_SVG_HEIGHT * ((1 - scale) / 2)})`}
			>
